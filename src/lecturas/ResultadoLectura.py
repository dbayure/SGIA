...
class ResultadoLectura(Resultado):
    ...
    def __init__(self, mensaje, fecha, idFactor, valor):
        """Constructor de la clase ResultadoLectura
            @type idFactor: int
            @param idFactor: Id del factor del que se obtuvo la lectura
            @type valor: float
            @param valor: Valor obtenido en la lectura."""
    ...
    def get_id_factor(self):
        """
        @rtype: int
        @return: Devuelve el id de dispositivo de un ResultadoLectura como un int"""
        return self.__idFactor

    def get_valor(self):
        """
        @rtype: float
        @return: Devuelve el valor obtenido en un ResultadoLectura como un float."""
        return self.__valor
